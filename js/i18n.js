    ...
    texts: {
        zh: {
            ...
            data: {
                ...
                // 通勤选项
                commute: {
                    car: { name: '🚗 开车', hint: '准时到达，油费 $5/次' },
                    bus: { name: '🚌 坐公交', hint: '-$5，20%概率迟到' },
                    walk: { name: '🚶 步行', hint: '健康+5，必定迟到' },
                ...
                // 通勤相关消息
                commute: {
                    car: { name: '🚗 开车', hint: (fuel, capacity) => `剩余油量 ${fuel}/${capacity}` },
                    car_repair: { name: '🔧 修车后开车', hint: (cost, insuranceInfo) => `-$${cost} ${insuranceInfo}，必定迟到` },
                    car_refuel: { name: '⛽ 加油并开车', hint: (cost) => `-$${cost}，加满油箱` },
                    bus: { name: '🚌 坐公交', hint: (cost, chance) => `-$${cost}，${chance}%概率迟到` },
                    walk: { name: '🚶 步行', hint: (health) => `健康+${health}，必定迟到` },
                    too_expensive: (cost) => `余额不足 (需要$${cost})`,
            ...
            ui_static: {
                ...
                assets_page: {
                    ...
                    cash_label: '现金',
                    portfolio_label: '投资总值',
                    total_assets_label: '净资产',
                    tab_gold: '🥇 黄金',
                    tab_stock: '📈 股票',
                    tab_crypto: '₿ 虚拟币',
                    news_loading: '加载中...',
